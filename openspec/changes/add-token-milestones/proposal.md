...
- The yard starts empty. No parked cars, no parked truck. The white bay lines and the truck bay lines stay, so it reads as an empty parking lot. Every sign stays as it is. The traffic on the roads is unchanged.
...
- The total travels with every session state as `machineTokens`, an optional field. A hub relays it untouched, so on a shared park each machine's lots show that machine's own total. The relay protocol number goes to 3, and `MIN_PROTOCOL` stays 2: a reporter that has not updated yet still connects, sends no total, and its lots stay empty until it does.
...
- A button top right opens a dialog with the whole ladder: one block per user, and per row an icon, the threshold and what is still to go. The total is a machine's, but the park names sessions by user, so the dialog does too.
- The forklift keeps its route. It now carries pallets between the dock and the truck bay, which is empty until 500M.
- README: a new section lists the ladder, the counting rule with the season start, and that the total is one number per machine that a hub shares with everyone on it.
- **Wire:** `SessionState` gains the optional `machineTokens`. A hub on protocol 3 still accepts a reporter on 2, so the fleet does not have to update in lockstep.
...
- `factory-scene`: new requirement "Token milestones" with the ladder, the in place rebuild and the overview dialog; "One factory per session" drops the parked cars and the parked truck from what every tier shares; "Hover details" adds the token line.

## Impact

- Server: `server/claude-reader.ts` (usage per assistant line, dedup key), new `server/usage-ledger.ts` (pure keep-max ledger with a total that ignores messages from before the season start), `server/session-tracker.ts` (`machineTokens` on every session state), `server/index.ts` (background scan of `~/.claude/projects/`, usage from tailed lines, a full read of a transcript first seen from the middle), `server/hub.ts` (protocol 3, minimum 2, the field copied through when a reporter sends it), `server/types.ts`. Tests for the parser, the ledger and the tracker field, with a fixture holding duplicate and resumed lines, and ledger cases for messages before and at the season start.
- Web: new `web/milestone-ladder.ts` (thresholds, labels, row lookup), new `web/milestones.ts` (builders for the extras, tick for the turbine and blimp), new `web/milestone-icons.ts` and `web/ladder-dialog.ts` (the overview), `web/lot.ts` (no default vehicles, rebuild key includes the milestone, tick the animated extras), `web/traffic.ts` (parked cars take a count), `web/machines.ts` (truck parts shared), `web/main.ts` (one total per user for the dialog), `web/tooltip.ts` and `web/sign-text.ts` (short token format), `web/index.html` and `web/style.css` (button and dialog, in both styles). Tests for the ladder lookup and the short format.
- Docs: `README.md`.
- Wire format: `SessionState.machineTokens?: number`. Protocol `3`, minimum `2`.
