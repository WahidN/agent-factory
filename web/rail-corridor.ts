...
import { RAIL_BRIDGE, RAIL_BRIDGE_SPAN, WAAL_EDGE } from "./city-plan.ts";
...
export function appendRailCorridor(builder: StaticBuilder, cells: readonly Cell[]) {
  for (const { from, to } of railSegmentsForCells(cells)) {
    const length = to - from;
    const center = (from + to) / 2;
    builder.box(ballast, [RAIL_X, 0.05, center], [8.6, 0.2, length]);
    ...
    // Continuous fencing prevents the corridor from reading as a road or
    // promenade; sparse catenary masts add the recognisable Dutch rail rhythm.
    for (const side of [-1, 1]) {
      const fenceX = RAIL_X + side * 4.55;
      builder.box(fenceSteel, [fenceX, 1.15, center], [0.1, 0.12, length]);
